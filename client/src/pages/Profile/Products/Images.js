...
// import { image } from '../../../../../server/config/cloudinaryConfig';
...
function Images({ selectedProduct, setShowProductForm, getData }) {
  ...
  const upload = async () => {
    try {
      ...
      formData.append("productId", selectedProduct._id); // make sure selectedProduct._id exists
      ...
      if (response.success) {
        ...
        setFile(null); //for disable the upload button
        getData();
      } else {
